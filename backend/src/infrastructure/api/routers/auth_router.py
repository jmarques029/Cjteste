...
from src.infrastructure.api.schemas.schemas import Token, ClienteRegister
...
def register(payload: ClienteRegister, db: Session = Depends(get_db)):
    ...
    existing_cliente = (
        ...
        .filter((ClienteModel.email == payload.email) | (ClienteModel.documento == payload.documento))
        .first()
    )
    if existing_cliente:
        raise HTTPException(
    ...
    return {"status": "sucesso", "cliente_id": new_cliente.id, "mensagem": "Cadastro realizado com sucesso."}
...
def login_for_access_token(
    ...
    if not is_admin:
        ...
        cliente = (
            db.query(ClienteModel)
            .filter((ClienteModel.email == form_data.username) | (ClienteModel.documento == form_data.username))
            .first()
        )
        if not cliente or not cliente.senha_hash or not verify_password(form_data.password, cliente.senha_hash):
            raise HTTPException(
