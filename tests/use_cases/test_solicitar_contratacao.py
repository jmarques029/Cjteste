...
def test_solicitar_contratacao_plano_inexistente():
    ...
    with pytest.raises(ValueError, match="Plano não encontrado"):
        use_case.executar(plano_id=99, cliente_dados={"nome": "A", "email": "a@a.com", "documento": "123"})
...
def test_solicitar_contratacao_sucesso():
    ...
    repo_cliente.salvar.return_value = Cliente(id=1, nome="A", email="a@a.com", documento="123")
    
    use_case = SolicitarContratacaoUseCase(repo_plano, repo_cliente)
    
    resultado = use_case.executar(plano_id=1, cliente_dados={"nome": "A", "email": "a@a.com", "documento": "123"})
